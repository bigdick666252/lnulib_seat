#修复v7可能存在的登录后等到时间过长导致cookie失效问题
import base64
...
def choose_it(driver, sit_avilable, idx, reading_room, day_type, max_attempts=5):
    """
    选择座位并预约时间，支持失败重试
        ...
        try:
            ...
            if reserve_box:
                ...
                if times_roll:
                    ...
                    if all(time in time_texts for time in full_day_times):
                        print(f"找到全天可预约座位: {i}")
                        found_full_day = True
                        day_type = 3
...
def perform_operations(driver, sit_avilable, idx, reading_room, day_type, account, password, options):
    ...
    if idx == 1:
        ...
        WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable(
                (By.XPATH, '//*[@class="el-tooltip room-name item" and text()="{}"]'.format(reading_room)))
        ).click()
...
def main():
    """主函数：循环登录多个账号并执行操作"""
    sit_avilable, day_type = None, None
    users = {"自定义": [account_password4, "三楼智慧研修空间", "30"]}
    user = "自定义"
    ...
    for idx, (account, password) in enumerate(total_accounts):
        try:
            ...
            profile_dir = tempfile.mkdtemp(prefix=f"edge_profile_{unique_suffix}_{idx}")
